 ...
 *
 * 220530 adapted to recent interface changes (mostly stricter C++ typing) by Torsten Beyer
 */
...
class AR62xxDevice final : public AbstractDevice {
  ...
  Port &port;
  //! Expected length of the message just receiving. - TB: unused -
 ...
 *
 * Original Method by Elmar Kümmel
 * Adaptation to new Interface copied from KRT2 driver.
 * Changed assert to reflect KRT2 driver approach. Removed "length" local variable and replaced with s.size()
 * TB
 **/
  ...
  {
 /**
  * changed asset to reflect KRT2 Driver
  **/
  ...
  bool doneok = AR620xParseString((const char *)data, (const size_t) s.size());
